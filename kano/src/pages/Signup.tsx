...
const Signup = () => {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      toast.success("Email verified securely!");
      navigate("/dashboard");
    }
    setLoading(false);
  };
      ...
      >
        ...
        {showOtp ? (
          <>
            ...
            <p className="text-muted-foreground text-center text-sm mb-8">We sent a 6-digit OTP to {email}</p>
            ...
            <form className="space-y-4" onSubmit={handleVerifyOtp}>
              <div>
                ...
                <input 
                  ...
                  className="input-glow w-full text-center tracking-widest text-lg font-bold" 
                  disabled={loading} 
                  required 
                  maxLength={6}
                />
          ...
          <>
            ...
            <form className="space-y-4" onSubmit={handleSignup}>
              <div>
                ...
                <input type="text" placeholder="John Doe" value={fullName} onChange={(e) => setFullName(e.target.value)} className="input-glow w-full" disabled={loading} required />
              </div>
              <div>
                <label className="text-sm font-medium mb-1.5 block">Email</label>
                <input type="email" placeholder="you@startup.com" value={email} onChange={(e) => setEmail(e.target.value)} className="input-glow w-full" disabled={loading} required />
              </div>
              <div>
                <label className="text-sm font-medium mb-1.5 block">Password</label>
                <div className="relative">
                  <input type={showPassword ? "text" : "password"} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className="input-glow w-full pr-10" disabled={loading} required />
