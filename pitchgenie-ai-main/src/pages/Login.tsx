      ...
      >
        ...
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          <div>
            ...
            <input type="email" placeholder="you@startup.com" className="input-glow w-full" />
          </div>
          <div>
            <label className="text-sm font-medium mb-1.5 block">Password</label>
            <div className="relative">
              <input type={showPassword ? "text" : "password"} placeholder="••••••••" className="input-glow w-full pr-10" />
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
                {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
              </button>
            </div>
          </div>
          <button type="submit" className="btn-gradient w-full">Sign In</button>
        </form>
